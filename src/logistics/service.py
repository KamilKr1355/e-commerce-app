...
def payment_succeed(db: Session, provider_payment_id: str):
  db_payment = db.query(Payment).filter(Payment.provider_payment_id == provider_payment_id).first()
...
def update_tracking(db: Session, shipment_id: int, tracking_number: str = None, status: Status = None):
...
def handle_webhook_event(db: Session, webhook_data: WebhookCreate):
    existing_event = db.query(WebhookEvent).filter(WebhookEvent.event_id == webhook_data.event_id).first()
    if existing_event:
